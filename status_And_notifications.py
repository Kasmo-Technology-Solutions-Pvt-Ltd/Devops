...
def notify(csv_file_path,source_branch,target_branch, execution_status, path,sf_params):
    ...
    if failures_exist:
        ...
        cursor.execute("SELECT subject, mail_body,email1  FROM DL_PC_EXT.APPROVER_MAIL_METADATA WHERE ENVIRONMENT='Dev'")
        row = cursor.fetchone()
        ...
        if "uat" in target_branch.lower():
            ...
            cursor.execute("SELECT subject, mail_body,email1  FROM DL_PC_EXT.APPROVER_MAIL_METADATA WHERE ENVIRONMENT='UAT'"        )
            row = cursor.fetchone()
        if "prod" in target_branch.lower():
            cursor.execute(
                "SELECT subject, mail_body,email1  FROM DL_PC_EXT.APPROVER_MAIL_METADATA WHERE ENVIRONMENT='PROD'")
            row = cursor.fetchone()
